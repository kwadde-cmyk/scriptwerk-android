...
function apkMimePlugin(): Plugin {
  function attach(server: { middlewares: { use: (fn: (req: { url?: string }, res: { setHeader: (k: string, v: string) => void }, next: () => void) => void) => void } }) {
    server.middlewares.use((req, res, next) => {
      ...
      if (pathOnly === "/Scriptwerk.apk") {
        res.setHeader("content-type", "application/vnd.android.package-archive");
        res.setHeader("content-disposition", 'attachment; filename="Scriptwerk.apk"');
      }
      next();
    });
  }
  return {
    name: "scriptwerk-apk-mime",
            ...
            routeRules: {
              "/Scriptwerk.apk": {
                headers: {
                  ...
                  "content-disposition": 'attachment; filename="Scriptwerk.apk"',
                },
              },
            },
